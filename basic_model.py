...
def create_model(train_set_size, input_length, num_epochs, batch_size):
    ...
    # lr = .001
    # adam0 = tf.keras.optimizers.Adam(learning_rate=lr)
...
def plots(history):
    ...
    plt.savefig(os.path.join(dirname, 'images/titanic/Accuracy_loss.png'))
    plt.close()

def test(batch_size):
    test = pd.read_csv(os.path.join(dirname, 'csvs/titanic/test.csv'), header=0)
    ...
    f = open(os.path.join(dirname, "csvs/titanic/output.csv"), "w")
    writer = csv.writer(f)
...
train = pd.read_csv(os.path.join(dirname, 'csvs/titanic/train.csv'), header=0)
